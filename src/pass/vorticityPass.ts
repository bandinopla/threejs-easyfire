...
export const vorticityPass = (
	...
	const velR = textures.velocity.sample(uvw.add(texel.mul(vec3(1, 0, 0)))).xyz; // Right (+X)
	const velL = textures.velocity.sample(uvw.sub(texel.mul(vec3(1, 0, 0)))).xyz; // Left (-X)
	const velU = textures.velocity.sample(uvw.add(texel.mul(vec3(0, 1, 0)))).xyz; // Up (+Y)
	const velD = textures.velocity.sample(uvw.sub(texel.mul(vec3(0, 1, 0)))).xyz; // Down (-Y)
	const velF = textures.velocity.sample(uvw.add(texel.mul(vec3(0, 0, 1)))).xyz; // Front (+Z)
	const velB = textures.velocity.sample(uvw.sub(texel.mul(vec3(0, 0, 1)))).xyz; // Back (-Z)
...
export const applyVorticity = (
	...
	const vortR = vorticityTex.sample(uvw.add(texel.mul(vec3(1, 0, 0)))).w;
	const vortL = vorticityTex.sample(uvw.sub(texel.mul(vec3(1, 0, 0)))).w;
	const vortU = vorticityTex.sample(uvw.add(texel.mul(vec3(0, 1, 0)))).w;
	const vortD = vorticityTex.sample(uvw.sub(texel.mul(vec3(0, 1, 0)))).w;
	const vortF = vorticityTex.sample(uvw.add(texel.mul(vec3(0, 0, 1)))).w;
	const vortB = vorticityTex.sample(uvw.sub(texel.mul(vec3(0, 0, 1)))).w;
